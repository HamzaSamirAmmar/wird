...
**Sends are DATA-ONLY** (`message.data`, never `message.notification`). A `notification` payload makes FCM's own service-worker handler display the push _in addition to_ our `onBackgroundMessage`, so every notification arrived twice. There must be exactly one displayer: ours. Tags are per-campaign (`campaign-<id>`) — a single shared tag made each notification silently replace the last.

Two duty notifications are automatic: a seeded **daily 08:00 (Damascus)** campaign with the `incomplete_today` audience, and the `notify_new_duties` trigger, which fires when a supervisor assigns a duty **for today** (after the 08:00 window has passed). The trigger sends a system ping (`{ auto: { kind, profileIds } }`, service-role only) rather than writing a campaign row: these are not authored, scheduled or disableable by anyone, and one campaign per assignment would bury the supervisor's real campaigns. It is **statement-level with a transition table** because `fanout_group_assignment()` inserts one duty per employee in a single statement — per-row would mean one HTTP request per employee.
